...
class VoiceClickService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        if (intent?.action == ACTION_STOP) { stop("Остановлено кнопкой STOP"); return START_NOT_STICKY }
        try { startForeground(1, notification()) } catch (_: RuntimeException) {
            stopRun("Нет доступа к микрофону. Разрешите его и запустите приложение снова."); return START_NOT_STICKY
        }
        if (intent == null || intent.getLongExtra(EXTRA_REQUEST, -1) != requestedSession) {
            if (!running) { stopForeground(STOP_FOREGROUND_REMOVE); stopSelf(startId) }
            return START_NOT_STICKY
        }
        if (running) return START_NOT_STICKY
        val clicker = ClickAccessibilityService.instance
        if (clicker == null || !clicker.hasValidPoint()) { stopRun("Разрешите нажатия и выберите точку"); return START_NOT_STICKY }
        if (!SpeechRecognizer.isRecognitionAvailable(this)) { stopRun("Распознавание речи недоступно. Включите системный сервис русской речи."); return START_NOT_STICKY }
    ...
    private fun createRecognizer() {
        ...
        speech.setRecognitionListener(object : RecognitionListener {
            override fun onReadyForSpeech(params: Bundle?) {
                ...
                val clicker = ClickAccessibilityService.instance ?: run { stopRun("Сервис нажатий отключён"); return }
                if (!clicksStarted) {
                    clicksStarted = true
                    clicker.startClicks((startDeadline - SystemClock.uptimeMillis()).coerceAtLeast(0))
                } else clicker.resumeClicks()
                SessionState.update(true, "Слушаю. Скажите «Лёша», чтобы остановить клики.")
            }
            override fun onPartialResults(results: Bundle?) { if (running) checkStop(results) }
            override fun onResults(results: Bundle?) { if (running && !checkStop(results)) scheduleListen() }
            ...
            override fun onError(error: Int) {
                ...
                if (error == SpeechRecognizer.ERROR_NO_MATCH || error == SpeechRecognizer.ERROR_SPEECH_TIMEOUT) {
                    failuresBeforeReady++
                    if (failuresBeforeReady < 20) scheduleListen(100) else stopRun("Микрофон не готов. Клики остановлены.")
                } else stopRun("Ошибка распознавания ($error). Клики остановлены; проверьте микрофон и связь.")
            }
    ...
    private fun listen() {
        ...
        } catch (_: RuntimeException) { stopRun("Не удалось включить распознавание. Клики остановлены.") }
    }

    private fun scheduleListen(delay: Long = 100) {
        if (!running) return
        handler.removeCallbacks(watchdog)
        handler.postDelayed({ if (running) listen() }, delay)
    }
    ...
    private fun notification(): Notification {
        ...
        return Notification.Builder(this, CHANNEL).setSmallIcon(android.R.drawable.ic_btn_speak_now)
            .setContentTitle("Лёша · кликер").setContentText("Скажите «Лёша» или нажмите STOP")
    ...
    companion object {
        ...
        fun stop(message: String) {
            ...
            if (service != null) service.stopRun(message) else SessionState.update(false, message)
        }
    }
}
